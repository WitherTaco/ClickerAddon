	...
	public class ClickerAddonGlobalItem : GlobalItem
	{
		public override bool Shoot(Item item, Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			if(ClickerCompat.IsClickerWeapon(item))
			{
				...
				if(player.GetModPlayer<ClickerAddonPlayer>().clonePrecursor)
				{
				...
				if(ClickerCompat.GetClickAmount(player) % 100 == 0 && player.GetModPlayer<ClickerAddonPlayer>().cloneOverclock)
				{
