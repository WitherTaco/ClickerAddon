...
{
	[AutoloadBossHead]
	public class MicePillar : ModNPC
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Mice Pillar");
		}
			...
			if (npc.ai[2] == 1f)
			{
				...
				if (Main.rand.NextBool(5) && npc.ai[1] < 150f)
				{
					for (int i = 0; i < 3; i++)
					{
						...
						{
							Dust dust6 = Main.dust[Dust.NewDust(position6, 4, 4, dustID, newColor: Color.Blue)];
		...
		public override void NPCLoot()
		{
			...
			{
				Item.NewItem((int)npc.position.X + Main.rand.Next(npc.width), (int)npc.position.Y + Main.rand.Next(npc.height), 2, 2, clickerClass.ItemType("MiceFragment"), Main.rand.Next(1, 4));
			}
